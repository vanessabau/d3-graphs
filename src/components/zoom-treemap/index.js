  ...
  useEffect(() => {
    ...
    const drawTreemap = (_DOM) => {
      ...
      function render(group, root) {
        ...
        node.append("title").text((d) => formatText(d).concat(`/${d.value}`));

        node
          .append("rect")
          .attr("id", (d) => (d.leafUid = d3.select("leaf")).id)
          .attr("fill", (d) => {
            let repo = d.data.name;
            return selectColor(repo);
          })
          .attr("stroke", "#fff");
      ...
      function position(group, root) {
        group
          .selectAll("g")
          .attr("transform", (d) =>
            d === root ? `translate(0,-35)` : `translate(${x(d.x0)},${y(d.y0)})`
          )
          .select("rect")
      ...
      let setTreemapProperties = d3.treemap().tile(tile);
